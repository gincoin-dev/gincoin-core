...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/gincoin.conf are parsed in qt/gincoin.cpp's main()
    ParseParameters(argc, argv);

    // Process help and version before taking care about datadir
    if (mapArgs.count("-?") || mapArgs.count("-h") ||  mapArgs.count("-help") || mapArgs.count("-version"))
    {
        std::string strUsage = _("Gincoin Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n";
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  gincoind [options]                     " + _("Start Gincoin Core Daemon") + "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "gincoin:"))
                fCommandLine = true;
        ...
        if (fDaemon)
        {
            fprintf(stdout, "Gincoin Core server starting\n");
...
int main(int argc, char* argv[])
{
    ...
    // Connect gincoind signal handlers
    noui_connect();
