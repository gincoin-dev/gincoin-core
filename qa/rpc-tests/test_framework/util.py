...
# Copyright (c) 2014-2017 The Gincoin Core developers
...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "gincoin.conf"), 'w') as f:
        f.write("regtest=1\n")
        f.write("rpcuser=rt\n")
...
def initialize_chain(test_dir):
    ...
    for i in range(4):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in gincoin.conf
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    ...
    if binary is None:
        binary = os.getenv("GINCOIND", "gincoind")
